...
import DeliveryActions from '../action/InvestmentRateActions';
import DeliveryStore from '../data/InvestmentRateStore';
...
var OrgBranchSelect= React.createClass({
        ...
        onServiceComplete: function(datas) {
            if(datas.operation === 'get'){
                var data = [];
                if(datas.object != null && datas.object.length > 0){
                    data = datas.object;
                }else{
        ...
        clear:function(){
         ...
         handleChange:function (value) {
            ...
            if(value =='无匹配结果'){
                ...
                this.setState({ value });
                DeliveryActions.searchSitesByFilter({orgName:value});
            }
