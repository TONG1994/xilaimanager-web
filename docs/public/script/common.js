  ...
  //获取登录者的类型
    getUserType:function () {
      let loginData = this.getLoginData(),
          userType = '';
      if(loginData){
        userType = loginData.staffInfo.orgType;
      }
      return userType;
    },
    // 邮箱服务器
    ...
    check: function (rule, value, page) {
        ...
        if (rule.required === true) {
            //用于选择框非空提示
            if( isEmpty && rule.customizeMethod ){
        ...
        if(rule.allowed){
            ...
            if(specialChar.test(value)){
                let allowArr = ['businessHours','orgAddress','manageCity','payNo','lng','lat','orgLocation','upTime','downTime','sendtime','textkey','notifyobject','picture','senderAddressDetail','receiverAddressDetail','area'];
                if(allowArr.indexOf(rule.id)===-1){
                  return '不能输入特殊符号';
                }
              }
        }
...
function FormVerify() {
    ...
    this.regEmail2 = /^\w[-\w.+]*@([A-Za-z0-9][-A-Za-z0-9]+\.)+[A-Za-z]{2,14}/;
...
var _isValidMethod = [
    ...
    {
        type: 'email2',
        isValid: function (str) { if (FormVerify.regEmail2.test(str) == false) return '请输入有效电子邮件地址!'; }
    },
    {
        type: 'idcard',
        isValid: isIdCard
