...
var PasswdPage = React.createClass({
    ...
    componentDidMount : function(){
        this.state.validRules = [
            {id: 'oldPasswd', desc:'原密码', required: true, max: '16',min:6,dataType:'passwd'},
            {id: 'newPasswd', desc:'新密码', required: true, max: '16',min:6,dataType:'passwd'},
        ];
    },

    handleOnChange : function(e) {
  		var passwd = this.state.passwd;
  		passwd[e.target.id] = e.target.value.replace(/^\s\s*/, '').replace(/\s\s*$/, '');;
